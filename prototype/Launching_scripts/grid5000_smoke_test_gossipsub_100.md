# GossipSub - Grid5000 100-node scoring experiment

Launch a minimal real libp2p GossipSub deployment on Grid5000 with 100 nodes,
peer scoring enabled, and 20% degraded peers.

Goal:
- reserve 100 Grid5000 nodes;
- mark 80 nodes as honest and 20 nodes as degraded;
...
EXP=/home/$LOGIN/results/gossipsub-scoring-100-$(date +%Y%m%d-%H%M%S)
NODES=$EXP/NodesFiles/nodes.csv
KEYS=$EXP/NodesFiles/keys
RESULTS=$EXP/results
TOPIC=gossipsub-scoring-100
...
## 9. Launch the 100-node experiment
...
for i in $(seq 0 $((NODE_COUNT - 1))); do
    ...
    ssh "${HOSTS[$i]}" \
        ...
        GOSSIPSUB_WORK_DIR='/tmp/gossipsub-scoring-100' \
        GOSSIPSUB_SLEEP_MARGIN=20 \
